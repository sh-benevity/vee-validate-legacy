...
export function findIndex<T>(arrayLike: ArrayLike<T>, predicate: (item: T, idx: number) => boolean): number {
  const array: Array<T> = Array.isArray(arrayLike) ? arrayLike : toArray(arrayLike)
 ...
 */
export function toArray<T>(arrayLike: ArrayLike<T>): Array<T> {
...
/* istanbul ignore next */
function _copyArray<T>(arrayLike: ArrayLike<T>): Array<T> {
  const array: Array<T> = []
...
export function values<T>(obj: { [x: string]: T }): Array<T> {
